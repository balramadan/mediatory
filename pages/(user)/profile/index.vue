            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                <label class="text-sm font-medium text-gray-600"
                  >No. Telepon</label
                >
                <p class="text-lg">{{ profileData.phone || "-" }}</p>
              </div>
              <div class="flex flex-col gap-1">
                <label class="text-sm font-medium text-gray-600">Status</label>
                <Tag
                  :value="formatStatus(profileData.status)"
                  :severity="getStatusSeverity(profileData.status)"
                />
              </div>
              <div>
                <label class="text-sm font-medium text-gray-600"
              ...
              <div>
                <label class="text-sm font-medium text-gray-600">ID User</label>
                <p class="font-mono text-sm">{{ profileData.user_id }}</p>
              </div>
            </div>
          </div>
        </div>
...
const fetchProfile = async () => {
  try {
    await $fetch(`/api/user/${userStore.user.id}`).then((res: any) => {
...
const changePassword = async () => {
  try {
    ...
    await $fetch(`/api/user/profile/${userStore.user.id}/change-password`, {
      method: "PUT",
      body: {
...
useSeoMeta({
  title: "Profil Saya | Mediawi",
  description: "Halaman profil dan pengaturan akun user",
});
...
:deep(.p-float-label .p-password .p-inputtext) {
  width: 100% !important;
}
